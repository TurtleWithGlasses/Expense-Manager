...
DB_FILE = config.get("database_path")
...
class Database:
    ...
    def add_budget(self, category: str, amount: float, period: str) -> None:
        """Add or update budget for a category"""
        cid = self.cat_id(category)
        if cid is None:
        ...
        existing = self.conn.execute(
            "SELECT id FROM budgets WHERE category_id = ?", (cid,)
        ).fetchone()
    ...
    def get_budget(self, category: str) -> Optional[Tuple[int, float, str]]:
        ...
        row = self.conn.execute(
            "SELECT id, amount, period FROM budgets WHERE category_id = ?", (cid,)
        ).fetchone()
        return row if row else None
    
    def get_budgets(self) -> List[Tuple[int, str, float, str]]:
    ...
    def delete_budget(self, category: str) -> None:
        ...
        cid = self.cat_id(category)
        if cid:
            self.conn.execute("DELETE FROM budgets WHERE category_id = ?", (cid,))
            self.conn.commit()
    
    def get_spent_amount(self, category: str, period: str) -> float:
        """Get spent amount for a category in the current period"""
        cid = self.cat_id(category)
        if cid is None:
            return 0.0
        ...
        row = self.conn.execute(
            ...
            WHERE category_id = ? AND date(date) BETWEEN date(?) AND date(?)
            """,
